...
import type { Project, ProjectsData } from '@/types/portfolio-types';
...
export default function Projects() {
  ...
  useEffect(() => {
    async function fetchProjects() {
      try {
        const projectPromises = projectsData.projects.slice(0, 4).map(async (projectUrl: string) => {
        ...
        const fetchedProjects = (await Promise.all(projectPromises)).filter(Boolean) as (Project & {
          tags: string[];
        })[];
  ...
  return (
    <section id='projects' className='max-w-360 mx-auto px-6 sm:px-8 md:px-12 lg:px-16 py-12 md:py-24'>
      ...
      >
        ...
        <p className='text-muted-foreground max-w-2xl mx-auto'>{projectsData.sectionDescription}</p>
      </motion.div>
      ...
      <div className='flex justify-center mt-8'>
        <Button asChild>
          <Link href={projectsData.allProjectsButton.href}>{projectsData.allProjectsButton.text}</Link>
        </Button>
      </div>
    </section>
  );
