...
export default function Testimonials() {
  ...
  const featuredTestimonials = testimonials.testimonials.filter((testimonial) => testimonial.featured);
  ...
  return (
    <section id='testimonials' className='container py-12 max-w-360 mx-auto px-6 sm:px-8 md:px-12 lg:px-16 md:py-24'>
      ...
      >
        <h2 className='text-3xl md:text-4xl font-bold'>{testimonials.sectionTitle}</h2>
        <p className='text-muted-foreground max-w-2xl mx-auto'>{testimonials.sectionDescription}</p>
      </motion.div>
      ...
      <div className='flex justify-center mt-8'>
        <Button asChild>
          <Link href={testimonials.viewAllButton?.href || '/#testimonials'}>{testimonials.viewAllButton?.text}</Link>
        </Button>
      </div>
    </section>
  );
